...
def db_insert(user: BankAccount) -> None:
    ...
    try:
        cu.execute("create table users (username, fullname, address, email, password, balance, account_history)")
    except:
        pass
...
def db_updatebalance(user: BankAccount) -> None:
    ...
    cu.execute("update users set balance = ?, account_history = ? where username = ?", (new_balance, 
                                                                                        account_history,
                                                                                        username))
    cx.commit()
    cu.close()
...
def db_recreateObject(username: str) -> BankAccount:
    ...
    print()
    return BankAccount(username=user[0],
                       fullname =user[1],
                       address = user[2],
                       email = user[3],
                       password = user[4],
